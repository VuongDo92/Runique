...
import com.plcoding.auth.domain.UserDataValidator.Companion.MIN_PASSWORD_LENGTH
    ...
    ObserveAsEvents(flow = viewModel.events) { events->
        when(events) {
            is RegisterEvent.Error -> {
                ...
                Toast.makeText(context, events.error.asString(context), Toast.LENGTH_LONG).show()
    ...
    GradientBackground {
        Column(
            ...
            RuniqueTextField(
                state = state.email,
                startIcon = EmailIcon,
                endIcon = if(state.emailValid) CheckIcon else null,
            ...
            Spacer(modifier = Modifier.height(16.dp))
            PasswordRequirement(
                text = stringResource(id = R.string.at_least_x_characters, MIN_PASSWORD_LENGTH),
...
fun PasswordRequirement(
    ...
    Row(
        ...
        Icon(
            imageVector = if (!isValid) CrossIcon else CheckIcon, contentDescription = null,
...
@Preview
@Composable
private fun RegisterStateScreenPreview() {
    RuniqueTheme {
        RegisterScreen(
            state = RegisterState(
